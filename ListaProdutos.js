...
export  class Produto extends Component {
		 ...
		 const  produt =  col. crianças [ 0 ]
         const  form = produt.children[0]
         const input = form.children[1]
         const categoria = input.children[0]		 
   
       const { checkitems } = this.produt;

    buscarArtistas = async (evento) => {
        evento.preventDefault()
	   
	   if (categoria.name == option.name){
       const produto = col.id(checkitems).filter(item => typeof checkitems[item] === this.option.name)
       return( produto);
	   }
	}
		       ...
		       <Col md={2}>     
                 <FormGroup>
                    ...
                    <Input type="select" name="select" id="exampleSelect">
                      <option onClick="buscarArtistas()" name= "Artistas"> Artistas</option>
                      <option onClick="buscarArtistas()" name= "Jogadores">Jogadores</option>
                      <option onClick="buscarArtistas()" name= "Politicos">Politicos</option>
