...
export default class Product extends Component {
    ...
    searchItem = (str) => {
        ...
        let itemsCategory = this.state.products.filter(x => x.category.toUpperCase().includes(str.toUpperCase())
        )


        let itemsImg = this.state.products.filter(x => x.img)
        let itemsPrice = this.state.products.filter(x => x.price)

        if(itemsCategory){
            this.state.itemsFilter.push(itemsImg.filter(x => x.img),itemsPrice.filter(x => x.price));
        }
        
                
       // this.state.itemsFilter.push(itemsCategory,itemsImg,itemsPrice);
        ...
        <Row >
            {this.state.products.map(item => (
                <Col md={11} className="bordList mb-2" id="play" onClick={this.searchItem} >
                    <Container className="themed-container " fluid={true}  >
                        <Col md={3} id="Player">

                            <CardImg className="p-3" width="100%" src={item.img} alt="Card image cap" />
                        </Col>
