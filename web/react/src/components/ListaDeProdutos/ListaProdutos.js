...
import { Card, CardTitle, CardText, Container, Col, CardImg, Row, Form, Label, Input } from 'reactstrap';
import {
    FaShoppingCart
} from "react-icons/fa";
import './style.css'
...
export  class Product extends Component {
		 ...
		 const  production =  col.children [ 0 ]
         const  form = produt.children[0]
         const input = form.children[1]
         const category = input.children[0]		 
   
       const { checkitems } = this.production;

    searchArtist = async (event) => {
        event.preventDefault()
	   
	   if (category.name == option.name){
       const card = col.id(checkitems).filter(item => typeof checkitems[item] === this.option.name)
       return( card);
	   }
	}
